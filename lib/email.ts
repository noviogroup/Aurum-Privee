...
export async function sendNewsletterConfirmation(input: { email: string; token: string }) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;color:#1d1c19;max-width:600px;margin:auto"><p style="color:#aa8140;letter-spacing:.18em;text-transform:uppercase;font-size:11px">Aurum Privée</p><h1 style="font-family:Georgia,serif;font-weight:400">Confirm your subscription.</h1><p>Confirm that you would like catalogue news and product updates from Aurum Privée.</p><p><a href="${escapeHtml(confirmationUrl.toString())}" style="display:inline-block;background:#aa8140;color:#fff;padding:14px 22px;text-decoration:none">Confirm my subscription</a></p><p style="color:#6d675e;font-size:13px">This link expires in 24 hours. If you did not request it, no action is needed.</p></div>`;
...
export async function sendContactInquiryNotification(input: {
  ...
  const html = `<div style="font-family:Arial,sans-serif;color:#1d1c19;max-width:640px;margin:auto"><p style="color:#aa8140;letter-spacing:.18em;text-transform:uppercase;font-size:11px">Aurum Privée · Trade support</p><h1 style="font-family:Georgia,serif;font-weight:400">New trade enquiry.</h1><p><strong>${escapeHtml(safeReference)}</strong></p><table style="width:100%;border-collapse:collapse">${rows}</table><div style="margin-top:20px;padding:20px;background:#f4efe7;white-space:pre-wrap">${escapeHtml(input.message)}</div></div>`;
  const delivery = await new Resend(apiKey).emails.send({
    from,
    to,
    replyTo: input.email,
    subject: `Aurum Privée enquiry ${safeReference}: ${input.topic}`.replace(/[\r\n]/g, "").slice(0, 150),
    html,
...
export async function sendQuoteRequestEmails(request: OperationsQuoteRequest) {
  ...
  const customerHtml = `<div style="font-family:Arial,sans-serif;color:#1d1c19;max-width:640px;margin:auto"><p style="color:#aa8140;letter-spacing:.18em;text-transform:uppercase;font-size:11px">Aurum Privée · Trade enquiries</p><h1 style="font-family:Georgia,serif;font-weight:400">Your quote request is with us.</h1><p>Hi ${escapeHtml(request.contactName)}, thank you for sending your selection for ${escapeHtml(request.companyName)}. Our trade team will review the products and quantities below, then contact you with pricing and availability or any questions. Keep this reference for follow-up.</p><p><strong>Reference ${escapeHtml(safeReference)}</strong></p><table style="width:100%;border-collapse:collapse">${lines}</table><p style="color:#6d675e;font-size:13px">This acknowledgement is not a price quote, order confirmation or reservation.</p></div>`;
  const merchantHtml = `<div style="font-family:Arial,sans-serif;color:#1d1c19;max-width:640px;margin:auto"><p style="color:#aa8140;letter-spacing:.18em;text-transform:uppercase;font-size:11px">Aurum Privée · Trade enquiries</p><h1 style="font-family:Georgia,serif;font-weight:400">New quote request.</h1><p><strong>${escapeHtml(safeReference)}</strong></p><table style="width:100%;border-collapse:collapse">${summaryRows}</table><table style="width:100%;border-collapse:collapse;margin-top:16px">${lines}</table>${note}<p>Review the request in the operations workspace. Confirm buyer details, availability and commercial terms before sending a written quote. Updating its status does not email the buyer.</p></div>`;
  const resend = new Resend(apiKey);
...
export async function sendFulfillmentEmail(input: {
  ...
  const content = input.status === "ready"
    ...
    : input.status === "fulfilled"
      ? { subject: `Order ${input.orderNumber} is complete`, title: "Your order is complete.", body: input.isDelivery ? "Your order has been delivered." : "Your order has been collected." }
