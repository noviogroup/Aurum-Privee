...
import { ArrowRight, ArrowSquareOut, CheckCircle, EnvelopeSimple, FileText, ImageSquare, LockKey, MagnifyingGlass, Package, PlugsConnected, SignOut, Storefront, Tag, Users, WarningCircle } from "@phosphor-icons/react";
...
export function OperationsCustomersConsole({ commerceProvider, initialCustomers }: { commerceProvider: CommerceProvider; initialCustomers: OperationsCustomers }) {
  ...
  return <div className="operations-app operations-customers-app">
    ...
    <section className="operations-workspace"><header className="operations-topbar"><div><Storefront size={18} weight="light" /><span>Trade catalogue</span></div><div>{wixManaged ? "Wix managed" : "Clienteling"}</div></header><div className="operations-page-head operations-customers-page-head"><h1>Customers</h1><p>{wixManaged ? "Wix owns customer and order records while hosted checkout is active." : "Recognize returning clients and serve every fragrance order with context."}</p></div>
      <div className="operations-frame operations-customers-frame"><section className="operations-queue operations-customers-queue">
        ...
        <div className="operations-customer-list">{visible.length ? visible.map((customer) => <button key={customer.email} className={selected?.email === customer.email ? "is-selected" : ""} onClick={() => choose(customer)}><span className="operations-avatar">{initials(customer.name)}</span><span className="operations-customer-name"><strong>{customer.name}</strong><small>{maskCustomerEmail(customer.email)}</small></span><span>{date(customer.lastOrderAt)}</span><span>{customer.orderCount}</span><span>{money(customer, customer.lifetimeSpend)}</span><span>{customer.vip ? "VIP" : "Not VIP"}</span></button>) : <div className="operations-empty"><Users size={30} weight="thin" /><h2>{wixManaged ? "Continue in Wix." : "No customers yet."}</h2><p>{wixManaged ? "Use Wix for customer and order history. This console will not create a second customer record." : "Customer profiles appear after a completed order."}</p></div>}</div>
        ...
        <div className="operations-customer-actions"><button className="operations-primary-button" disabled={busy || data.preview || wixManaged} onClick={save}>{busy ? "Saving" : wixManaged ? "Managed in Wix" : data.preview ? "Connect database" : "Save client profile"}</button><Link href={`/operations?order=${selected.orders[0]?.id || ""}`}>Open latest order</Link></div><p className="operations-customer-privacy"><LockKey size={14} />Staff notes stay inside Aurum Privée and are never sent to {wixManaged ? "Wix" : "Loyverse"}.</p>
      </> : <div className="operations-empty"><Users size={30} weight="thin" /><h2>No local profile selected.</h2><p>{wixManaged ? "Wix remains the customer record authority." : "Choose a customer to review their profile."}</p></div>}</aside></div>
    </section>
  </div>;
}
