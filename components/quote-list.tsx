...
export function QuoteList() {
  ...
  const productIdsKey = JSON.stringify(savedIds);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { response, data } = await requestJson<unknown>(`/api/catalog?ids=${encodeURIComponent(productIds.join(","))}`, { signal: controller.signal });
        ...
        setProducts(productIds.map((id) => byId.get(id)).filter((product): product is PublicProduct => Boolean(product)));
      } catch {
  ...
  if (reference) return (
    <div className="quote-list-page page-top">
      <section className="quote-success section-shell" aria-labelledby="quote-success-title">
        ...
        <p>Your quote request has been saved. Keep your reference above for follow-up. The trade team will review availability, quantities and commercial terms before responding.</p>
  ...
  return (
    <div className="quote-list-page page-top">
      <section className="saved-heading section-shell entrance">
        <h1>Quote list</h1>
        <p>Review your products and quantities, then send your selection to the trade team for pricing and availability.</p>
      </section>
      {!hydrated || loading ? <div className="saved-status section-shell" role="status">Preparing your quote list…</div> : loadError ? (
        ...
        <div className="quote-workspace section-shell">
          ...
          <form className="quote-request-form" onSubmit={submit}>
            <div><p className="utility-label">Trade enquiry</p><h2>Request your quote</h2><p>Tell us who you are buying for. We’ll review your selection and confirm pricing, minimums and availability directly.</p></div>
            ...
            <label>Request notes <span>Optional</span><textarea maxLength={2000} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder="Include your required date, delivery location and any previous quote reference." /></label>
            ...
            <small>This request is not an order, reservation or binding quotation. Read our <Link href="/pages/trade-program">trade process</Link> and <Link href="/pages/privacy">privacy notice</Link>.</small>
          </form>
        </div>
      )}
    </div>
