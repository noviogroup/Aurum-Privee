...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({ orderId: order.id, orderNumber: order.order_number, status: order.fulfillment_status, emailSent: false, error: "The fulfillment state was saved, but its email needs retry." }, { status: 502 });
    }
  } catch (error) {
    ...
    console.error("Order fulfillment transition failed", error);
